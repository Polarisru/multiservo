...
void CLK_Init(void)
{
  ...
  // Switch to 48MHz clock, prescaler = 2 (OSCCTRL_OSC48MDIV_DIV(1))
  //OSCCTRL->OSC48MCTRL.reg = (CONF_OSC48M_ENABLE << OSCCTRL_OSC48MCTRL_ENABLE_Pos);
...
//  while (OSCCTRL->OSC48MSYNCBUSY.reg);
  ...
  // don't use prescaler (not enabled)
