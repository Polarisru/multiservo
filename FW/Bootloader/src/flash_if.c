...
/**
  * @brief  Check if address is start of any sector
  * @param  Address: address to check
  * @retval 0: This is not starting address
  *         1: This is starting address
  */
...
/**
  * @brief  This function does an erase of one sector
...
{
  uint32_t StartSector = FLASH_If_GetSector(Address);
...
{
  uint32_t UserStartSector, UserEndSector, i;

  /* Get the sector where start the user flash area */
  UserStartSector = FLASH_If_GetSector(APPLICATION_ADDRESS);
  UserEndSector = FLASH_If_GetSector(APPLICATION_ADDRESS + Size);
